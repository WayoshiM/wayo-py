...
class LineupCog(commands.Cog, name='TPIRLineups'):
            ...
            if retro:
                ...
                pgps = [
                    PGPlaying(
                        (
                            f"Back to '{m.group()}"
                            if (m := re.match(f'(7[2-{BACK_YEAR}])', arg))
                            else f"Back to '7{BACK_YEAR}"
                        )
                        if pg == PG.BackToXX
                        else pg.sheetName,
    ...
    async def lineupProd(self, ctx, *, production_numbers):
        ...
        for time in ('daytime', 'primetime', 'syndicated', 'unaired'):
            sub_df = await trim_query(self.cs.get(time).lazy().filter(pl.col('PROD').is_in(prods)))
            if sub_df.height:
