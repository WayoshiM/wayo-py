...
class WheelCompendium:
    ...
    def __init__(self, *, loop=None, debug: bool = False):
        ...
        self.dfs = {t: None for t in ('syndicated', 'primetime', 'sched', 'kids', 'daytime', 'au', 'gb')}
        ...
        self._df_sched_dict = SortedDict()
        self.coverage = None
        ...
        load = self.load(
            ...
            + ['primetime', 'kids', 'daytime', 'au', 'gb', 'choicesprimetime']
    ...
    async def load(self, pages: Collection[int | str]):
        ...
        if changed_syndicated:
            self.dfs['syndicated'] = pl.concat(self._df_syndicated_dict.values()).collect(streaming=True).lazy()
            choices_update = any(s >= 35 for s in changed_syndicated)

        for k in self.dfs.keys():
            if k in pages:
                self.dfs[k] = self.dfs[k].collect(streaming=True).lazy()

        # streaming=True on this particular concat scrambled order (bug??)
        self.dfs['sched'] = pl.concat(self._df_sched_dict.values()).collect().lazy()
        self.join_cache['sched'] = (
            self.dfs['syndicated']
            .join(self.dfs['sched'].drop(cs.contains('DATE')), on='EP', how='left')
            .collect()
            .lazy()
        )
        ...
        for k, v in self._internal_df_choices.items():
            ...
            for k2, f, cdf in cdfs:
                ...
                if missing.height:
                    self.sanity_checks.add(
                        f'The following EPS have BRs in "BR choices" but not in "{k}" (double-check HTML formatting): '
                        + str(missing.get_column('EP').to_list())
                    )

                mismatch = df.filter(
                    pl.concat_list(cs.matches('^CAT\d$').cast(str)).list.get(pl.col('C') - 1) != pl.col('CATEGORY')
                ).collect()
                if mismatch.height:
                    self.sanity_checks.add(
                        f'The following EPS have BRs in "BR choices" with incorrectly highlighted choice, compared to "{k}"): '
                        + str(mismatch.get_column('EP').to_list())
                    )
            ...
            if choices_update and k == 'syndicated':
                self.df_choices['syndicated'] = pl.concat(built_dfs).collect(streaming=True).lazy()
            else:
                self.df_choices['primetime'] = built_dfs[0].collect(streaming=True).lazy()
    ...
    async def _load_season(self, season: int | str) -> bool:
        ...
        try:
            df = pl.read_csv(
                ...
                dtypes={'EP': pl.UInt16, 'ROUND': pl.Categorical},
        ...
        if not is_syn and re.match('choices\d0', season):
            self._internal_df_choices['syndicated'][season[-2]] = df.select(
                'EP',
                pl.col('DATE').str.strptime(pl.Date, "%m/%d/%y"),
                pl.col('C').cast(pl.UInt8),
                cs.matches('^CAT\d?$').cast(pl.Categorical),
            )
        if not is_syn and re.match('sched\d0', season):
            self._df_sched_dict[season[-2]] = df.select(
                ...
                pl.col('THEME').cast(pl.Categorical),
            )
            ...
            if is_syn or season == 'primetime':
                ...
                date_one_to_one = (
                    ...
                    .filter(pl.col('EP').list.lengths() > 1)
                    .collect()
                )
                ...
                ep_one_to_one = (
                    ...
                    .filter(pl.col('DATE').list.lengths() > 1)
                    .collect()
                )
            ...
            if is_syn:
                ...
                c_exprs = [
                    ...
                    .cast(pl.UInt64),
                    pl.col('DATE').str.strptime(pl.Date, '%m/%d/%y'),
                    pl.col('UNC').fill_null('').cast(pl.Categorical).cat.set_ordering('lexical'),
