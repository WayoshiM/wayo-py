...
cs = ConflictSheet(
    lambda fn: open(os.path.expanduser("~/Dropbox/heroku/wayo-py/" + fn), "rb"),
    sf,
    False,
)
# print(datetime.datetime.now())
...
vv = (
    ...
    .select(
        [
            pl.col("PG"),
            pl.fold(pl.lit(0), operator.add, pl.col(f"^PG[12]$")).alias("sum"),
        ]
    )
...
xxxxx = (
    ...
    .with_columns(
        [
            pl.Series("S", ["S1-17", "S18-34", "S35-51"]),
...
y = cs.concurrence_query(
    P.closed(46, 50),
    "daytime",
    (PG.Plinko, PG.RangeGame),
    (frozenset((8,)), frozenset((1, 16))),
)
print(y.collect())
