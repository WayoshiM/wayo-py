        ...
        if page.status_code != 200:
            if page.status_code == 403:
                try:
                    ...
                    page = await self.asession.get(
                        f'https://buyavowel.boards.net/page/compendium{page_suffix}', proxies=proxies
                    )
                    ...
                    if page.status_code != 200:
                        if page.status_code == 403:
                            raise ValueError(
                                f"The IP I'm currently on is banned by Proboards. I attempted to try one of two static IP (limited to 250 free uses a month) as backup and it also failed (perhaps Wayo goofed some code somewhere, as he is wont to do). Ask Wayoshi to manually restart the bot, as this should change the IP."
                            )
                        else:
                            raise ValueError(
                                f'Could not find webpage on my end for page "{page_suffix}". Getting status code {page.status_code}. (Used a static IP call)'
                            )
            else:
                raise ValueError(
                    f'Could not find webpage on my end for page "{page_suffix}". Getting status code {page.status_code}.'
        ...
        with StringIO() as s:
            ...
            for row in trs:
                ...
                if m and m2:
                    ...
                    raise ValueError(f'Improperly formatted date or showno: "{date_used}", "{showno}"')

            return s.getvalue().encode()

    def _dl_sched(self, trs):
        with StringIO() as s:
            f = csv.writer(s)
            f.writerow(['EP', 'DATE', 'RED', 'YELLOW', 'BLUE', 'THEME'])
    ...
    for cond in options.conditions:
        ...
        match words:
            case ['BONUS' | 'B']:
                ...
                f = (pl.col(col).str.lengths() > 0) & (pl.exclude('CATEGORY').cast(str).str.contains('CROSSWORD'))
                cd = 'has a bonus'
            case ['BONUS' | 'B' as col, '1' | '0' | 'YES' | 'NO' | 'Y' | 'N' | 'T' | 'F' | 'TRUE' | 'FALSE' as b]:
                ...
                f = (pl.col('BONUS').str.lengths() > 0) & (pl.exclude('CATEGORY').cast(str).str.contains('CROSSWORD'))
                if not b:
                    f = f.not_()
            ...
            case ['DATE' | 'D' as col, *e]:
                ...
                if options.time in ('kids', 'daytime', 'au', 'gb'):
                    ...
                    cd = f'{col} is {cd}'
            case ['LENGTH' | 'LC' | 'L', *e]:
                f, cd, _ = build_int_expression(pl.col('PUZZLE').str.extract_all('[A-Z]').list.lengths(), e)
                cd = f'length is {cd}'
            case ['LENGTH_UNIQUE' | 'LCU' | 'LU', *e]:
                f, cd, _ = build_int_expression(pl.col('PUZZLE').str.extract_all('[A-Z]').list.unique().list.lengths(), e)
        ...
        match words:
            ...
            case ['UC' | 'UNCHOSEN' | 'NC' | 'NOTCHOSEN' | 'NOT_CHOSEN', regex]:
                ...
                f = pl.all_horizontal(
                    ...
                    pl.exclude('CHOSEN').cast(str).str.contains(regex),
                )
            ...
            case [
                ...
                if p_q.startswith('L'):
                    f = pl.all_horizontal(
                        ...
                        pl.exclude('CHOSEN').cast(str).str.contains(lit, literal=True),
                    )
                    verb = 'contains'
...
def gen_sched_expr(words):
    match words:
        ...
        case ['CONTESTANT' | 'CON' | 'PODIUM' | 'P', regex]:
            ...
            cd = f'any PODIUM matches "{regex}"'
        case ['EP', *e]:
            f, cd, _ = build_int_expression(pl.col('EP'), e)
            cd = f'EP is {cd}'
        case [
            'DATE' | 'D' as col,
            'YEAR' | 'Y' | 'MONTH' | 'M' | 'DAY' | 'D' | 'DOW' | 'WKDAY' | 'WEEKDAY' as dt_q,
        ...
        case ['DATE_STR' | 'DS', regex]:
            ...
            cd = f'DATE is uncertain and matches "{regex}"'
        case _:
            raise ValueError(f'Malformed condition: {cond}')
